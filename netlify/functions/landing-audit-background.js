...
- Do NOT flag a missing meta description as critical or high severity. Meta descriptions affect organic search snippet display and brand follow-up search CTR — they have no direct impact on Google Ads Quality Score, Landing Page Experience, or paid media conversion rate. If you flag a missing meta description at all, use severity "low" or "suggest" and frame it explicitly as SEO housekeeping, not a paid-media conversion barrier. A PPC specialist reading this report should not see meta description above fold, trust, speed, or CTA issues.
...
function buildReport (url, parsed, psiMobile, psiDesktop, claude, isHttps, fetchError, psiStatus) {
  ...
  if (!isEcommerce) {
    if (parsed.formFieldCount >= 7)
      findings.push({ category: 'conversion_architecture', severity: 'high', title: `${parsed.formFieldCount}-field form — high abandonment risk`, detail: `A ${parsed.formFieldCount}-field form is a significant barrier for users who clicked an ad in a moment of intent. For paid traffic especially, every field is a reason to leave — form length consistently ranks as one of the highest-impact friction points on landing pages.`, fix: 'Reduce to 3 fields maximum for the initial enquiry. You can collect additional details in the follow-up process after the first conversion event.', impact: "Reducing to 3 fields is the highest-leverage friction reduction on a form-based landing page — field reduction consistently outperforms copy and design changes for CPA reduction." });
  ...
  if (claude.message_match_strength === 'weak' && !claudeCovers('landing_page_experience', ['message match', 'ad copy', 'headline']))
    findings.push({ category: 'landing_page_experience', severity: 'high', title: 'Weak message match between page and likely ad copy', detail: `Message match is one of the most predictable conversion levers in paid search. When the language on the landing page echoes the ad that drove the click, post-click engagement and conversion rate both improve. Weak alignment between the page headline and the ad intent also reduces Quality Score.`, fix: 'Mirror your primary ad headline in the page H1, and use the same language (keyword, offer framing, call to action) in your above-fold copy. Visitors should feel they landed exactly where they expected.', impact: "Strong message match between ad and page is one of the most reliable conversion levers in paid search — it directly strengthens Quality Score and reduces CPCs." });

  // ── CTA specificity ────────────────────────────────────────────────────────
  if (claude.cta_specificity === 'generic' && parsed.hasAboveFoldCta && !claudeCovers('conversion_architecture', ['cta', 'call to action', 'button']))
    findings.push({ category: 'conversion_architecture', severity: 'suggest', title: 'CTA could be more specific', detail: `Generic CTAs like "Submit" or "Click here" consistently underperform specific action-oriented language. A CTA that tells the visitor exactly what happens next — "Book Your Free Trial", "Get Your Quote" — reduces hesitation at the point of conversion.`, fix: `Replace generic CTA text with a specific outcome: "Book Your Free Trial", "Get Your Quote in 2 Minutes", or similar language tied to your conversion goal.`, impact: "Specific CTAs consistently outperform generic button text — this is a one-line copy change with measurable conversion rate impact." });
  ...
  if (claude.social_proof_quality === 'none' && parsed.testimonialBlockCount === 0 && !parsed.starRatingPresent && !claudeCovers('trust_signals', ['social proof', 'testimonial', 'review', 'rating']))
    ...
    findings.push({ category: 'trust_signals', severity: 'suggest', title: 'Social proof could be stronger', detail: `Anonymous testimonials or generic review statements carry less weight than named, specific social proof. Testimonials with a name, role, and specific outcome are significantly more credible than generic quotes — they give the reader a person to identify with.`, fix: 'Upgrade anonymous testimonials to include the reviewer\'s name and specific outcome ("My son went from nervous to confident in 4 weeks — Jane P., parent"). Photos increase trust further.', impact: "Named, specific testimonials carry more weight than anonymous ones — worth upgrading without needing to change page structure." });
  ...
  const priorityActions = [...findings]
    ...
    .slice(0, 3)
    .map(f => ({ severity: f.severity, category: f.category, title: f.title, detail: f.detail || '', fix: f.fix || '', impact: f.impact || '' }));
